...
<Card.Root class='flex flex-col gap-2 justify-end'>
    {#if loadingUsername}
    <Skeleton class='w-60 h-5' />
    {:else if (showUsernameInput)}
        <div class="flex gap-2">
            <Input bind:value={usernameValue} placeholder={username || 'What should you be called on Y?'}/>
    ...
    <Button on:click={post} disabled={content.length === 0 || !username || buttonLoading} class='max-w-36 w-full ml-auto'>
        {#if buttonLoading}
